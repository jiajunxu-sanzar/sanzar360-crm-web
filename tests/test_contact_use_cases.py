...
def test_create_empty_contact_bootstraps_when_no_sheet_headers() -> None:
    ...
    new_df, contact_id, verify = create_empty_contact(df, sheets)  # type: ignore[arg-type]
    assert len(new_df) == 1
    assert contact_id
...
def test_save_contact_by_id_updates_dataframe_and_calls_partial_save() -> None:
    ...
    updated, verify = save_contact_by_id(
        ...
        values={"contact_id": contact_id, "nombre": "Cliente Demo", "municipio": "Sevilla"},
...
def test_save_contact_by_id_rejects_contact_id_mismatch() -> None:
    sheets = _FakeSheets(simulate_sheet_headers=False)
    df = empty_contacts_dataframe()
    new_df, contact_id, _verify = create_empty_contact(df, sheets)  # type: ignore[arg-type]
    row_idx = new_df.index[0]
    try:
        save_contact_by_id(
            ...
            row_idx=row_idx,
            contact_id=contact_id,
            values={"contact_id": "", "nombre": "X"},
            sheets=sheets,  # type: ignore[arg-type]
        )
        assert False, "expected mismatch RuntimeError"
